...
char Tokenizer::peek() const { return isAtEnd() ? '\0' : input_[pos_]; }
...
Token Tokenizer::scanNumber() {
    ...
    double v = 0;
    try { v = std::stod(raw); }
    catch (...) { throw CalculatorException("Invalid number: " + raw); }

    return Token{TokenType::NUMBER, v, "", 0};
}
...
Token Tokenizer::scanIdentifier() {
    ...
    if (CONSTANTS.count(id)) {
        ...
        return Token{TokenType::CONSTANT, val, id, 0};
    }
    if (FUNCTIONS.count(id))
        return Token{TokenType::FUNCTION, 0, id, 0};
...
Token Tokenizer::next() {
    skipWhitespace();
    if (isAtEnd()) return Token{TokenType::END, 0, "", 0};
    ...
    if (c == '+' || c == '-' || c == '*' || c == '/' || c == '%' || c == '^')
        return Token{TokenType::OPERATOR, 0, std::string(1, c), 0};

    if (c == '(') return Token{TokenType::LEFT_PAREN,  0, "(", 0};
    if (c == ')') return Token{TokenType::RIGHT_PAREN, 0, ")", 0};
    if (c == ',') return Token{TokenType::COMMA,        0, ",", 0};
